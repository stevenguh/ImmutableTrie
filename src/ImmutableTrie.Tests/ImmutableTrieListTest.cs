...
{
  public class ImmutableTrieListTest : ImmutableTrieListTestBase
  {
    ...
    public void RandomOperationsTest()
    {
      ...
      var actual = ImmutableTrieList<int>.Empty;
    ...
    {
      var empty = ImmutableTrieList<GenericParameterHelper>.Empty;
      Assert.Same(empty, ImmutableTrieList<GenericParameterHelper>.Empty);
    ...
    public void AddAndIndexerTest()
    {
      ...
      var bulkList = ImmutableTrieList<int>.Empty.AddRange(Enumerable.Range(1, 10).Select(i => i * 10));
    ...
    public void AddRangeTest()
    {
      ...
      list = list.AddRange(ImmutableTrieList<int>.Empty.AddRange(new[] { 6, 7, 8 }));
      list = list.AddRange(new int[0]);
      list = list.AddRange(ImmutableTrieList<int>.Empty.AddRange(Enumerable.Range(9, 1000)));
    ...
    /// <summary>
    /// Makes sure Add, Set, Pop and Get don't mutate the pre-existing record.
    /// </summary>
    [Fact]
    ...
    [Fact]
    public void GetRangeStructuralTest()
    {
      const int itemsInTrie = 32768;
      const int itemsInTail = 24;
      const int total = itemsInTrie + itemsInTail;
      // Test get range with start and end in tail
      TestGetRange((list, original) =>
        {
          var rangedList = list.GetRange(itemsInTrie, 12); ;
          var rangedCopy = rangedList.ToList();
          Assert.Equal(original.Skip(itemsInTrie).Take(12), rangedList);
          Assert.Equal(original.Skip(itemsInTrie + 1).Take(12), list.GetRange(itemsInTrie + 1, 12)); // additional fuzz
          return (rangedList, rangedCopy);
        },
        itemsInTrie,
        itemsInTail
      );

      // Test get range with start in trie, end in tail
      TestGetRange((list, original) =>
        {
          var rangedList = list.GetRange(1, total - 4);
          var rangedCopy = rangedList.ToList();
          Assert.Equal(original.Skip(1).Take(total - 4), rangedList);
          Assert.Equal(original.Skip(31).Take(36), list.GetRange(31, 36)); // additional fuzz
          return (rangedList, rangedCopy);
        },
        itemsInTrie,
        itemsInTail
      ...
      // Test get range with start in one node, end in another node
      TestGetRange((list, original) =>
        {
          var rangedList = list.GetRange(1, 100);
          var rangedCopy = rangedList.ToList();
          Assert.Equal(original.Skip(1).Take(100), rangedList);
          return (rangedList, rangedCopy);
        },
        itemsInTrie,
        itemsInTail
      ...
      // Test get range with start in one node end in the same node
      TestGetRange((list, original) =>
        {
          var rangedList = list.GetRange(1, 12);
          var rangedCopy = rangedList.ToList();
          Assert.Equal(original.Skip(1).Take(12), rangedList);
          return (rangedList, rangedCopy);
        },
        itemsInTrie,
        itemsInTail
    ...
    {
      var list = ImmutableTrieList<GenericParameterHelper>.Empty;
    ...
    public void IndexOfAndContainsTest()
    {
      ...
      var list = ImmutableTrieList<string>.Empty;
    ...
    public void ReplaceTest()
    {
      ...
      var list = ImmutableTrieList<int>.Empty.Add(3).Add(5).Add(8);
      ...
      list = ImmutableTrieList<int>.Empty.Add(3).Add(3).Add(5);
    ...
    {
      var list = ImmutableTrieList.Create(new Person { Name = "Andrew", Age = 20 });
    ...
    {
      Assert.Throws<ArgumentException>("oldValue", () => ImmutableTrieList<int>.Empty.Replace(5, 3));
    }

    [Fact]
    public void EqualsTest()
    {
      Assert.False(ImmutableTrieList<int>.Empty.Equals(null));
      Assert.False(ImmutableTrieList<int>.Empty.Equals("hi"));
      Assert.True(ImmutableTrieList<int>.Empty.Equals(ImmutableTrieList<int>.Empty));
      Assert.False(ImmutableTrieList<int>.Empty.Add(3).Equals(ImmutableTrieList<int>.Empty.Add(3)));
    }
    ...
    [Fact]
    public void ToImmutableTrieList()
    {
      ImmutableTrieList<string> list = new[] { "a", "b" }.ToImmutableTrieList();
      Assert.Equal(2, list.Count);

      list = new[] { "a", "b" }.ToImmutableTrieList();
      Assert.Equal(2, list.Count);
    }
    ...
    {
      Assert.Throws<ArgumentNullException>("match", () => ImmutableTrieList<int>.Empty.RemoveAll(null));
    }
    ...
    {
      IList list = ImmutableTrieList.Create<int>();
    ...
    /*
            ...
            {
                DebuggerAttributes.ValidateDebuggerDisplayReferences(ImmutableTrieList.Create<int>());
                ImmutableTrieList<double> list = ImmutableTrieList.Create<double>(1, 2, 3);
                DebuggerAttributeInfo info = DebuggerAttributes.ValidateDebuggerTypeProxyProperties(list);

                object rootNode = DebuggerAttributes.GetFieldValue(ImmutableTrieList.Create<string>("1", "2", "3"), "_root");
            ...
            {
                Type proxyType = DebuggerAttributes.GetProxyType(ImmutableTrieList.Create<double>());
    ...
    */
        ...
        public void UsableWithCollectibleAssemblies()
        {
            ...
            var createMethod = typeof(ImmutableTrieList).GetMethods().Where(m => m.Name == "Create" && m.GetParameters().Length == 0).Single().MakeGenericMethod(dummType.AsType());
    ...
    /*
            ...
            public void ItemRef()
            {
                var list = new[] { 1, 2, 3 }.ToImmutableTrieList();
    ...
    */
    ...
    {
      return ImmutableTrieList<T>.Empty.AddRange(contents);
    }
    ...
    private void TestGetRange(
      Func<ImmutableTrieList<int>, List<int>, (ImmutableTrieList<int> rangedList, List<int> rangedCopy)> testFunc,
      int itemsInTrie,
      int itemsInTail)
    {
      int total = itemsInTrie + itemsInTail;
      var list = ImmutableTrieList.CreateRange(Enumerable.Range(0, total));
      var original = list.ToList();

      var (rangedList, rangedCopy) = testFunc(list, original);

      // Make sure add, set, and pop can still work as normal.
      Assert.Equal(
        rangedCopy.Concat(Enumerable.Range(0, 1024)),
        rangedList.AddRange(Enumerable.Range(0, 1024))
      );

      var tempList = rangedList;
      for (int i = 0; i < tempList.Count; i++)
      {
        tempList = tempList.SetItem(i, -1);
        Assert.Equal(-1, tempList[i]);
      }
      ...
      for (int i = 0; i < tempList.Count; i++)
      {
        ...
        Assert.Equal(oldCount - 1, tempList.Count);
      }

      Assert.Equal(original, list); // make sure the original stay unchanged
      Assert.Equal(rangedCopy, rangedList);
    }
